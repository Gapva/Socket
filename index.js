...
const { JSDOM } = jsdom;
var document = new JSDOM("./public/index.html").window.document;
	...
	socket.on('join', (data) => {
		...
		if (!Object.keys(allsockets).includes(room)) {
			...
			toRoom(room).emit('message', {
				name: 'server',
				message: `${socket.proto.id} slid into existence. Welcome!`
			});
			socket.emit('message', {
				name: 'server',
				message: `Welcome to Socket! You are currently in room "${socket.proto.room}". To change your nickname, type /nick [name]. To join a different room, type /join [room]. Type /help to see more commands`
			});
			...
			if (query({
				...
				toRoom(socket.proto.room).emit('message', {
					name: 'server',
					message: `${socket.proto.name} confirmed themselves as a Socket administrator. I hope you didn't do something horrible, because if you did, I feel so bad for you.`
				});
			}
		} else {
			socket.emit('message', {
				...
				switch (message.split(' ')[0]) {
					case '/whois':
						...
						if (selectedSocket) {
							...
							socket.emit('message', {
								name: 'server',
								message: `Error: User/Room ${message.split(' ')[1]} isn't real. Get real by making a room using /join [room name] or maybe invite someone to Socket (please)`
							});
						}
						break;
					case '/kick':
						...
						if (socket.proto.admin) {
							if (selectedSocket) {
								...
								socket.emit('message', {
									name: 'server',
									message: `Error: User ${message.split(' ')[1]} isn't real. Get real by inviting them to Socket (please)`
								});
							}
						} else {
							socket.emit('message', {
								name: 'server',
								message: `Error: Epic perms fail (NO perms perms :pensive:)`
							});
						}
						break;
					case '/deop':
						...
						if (socket.proto.admin) {
							if (selectedSocket && selectedSocket.proto.id !== socket.proto.id) {
								...
								toRoom(socket.proto.room).emit('message', {
									name: 'server',
									message: `${selectedSocket.proto.name} is no longer an administrator. Still, don't act-up, people.`
								});
							} else {
								socket.emit('message', {
									name: 'server',
					...
					case '/op':
						...
						if (socket.proto.admin) {
							if (selectedSocket) {
								...
								toRoom(socket.proto.room).emit('message', {
									name: 'server',
									message: `${selectedSocket.proto.name} confirmed themselves as a Socket administrator. I hope you didn't do something horrible, because if you did, I feel so bad for you.`
								});
							} else {
								socket.emit('message', {
									name: 'server',
					...
					case '/unmute':
						...
						if (socket.proto.admin) {
							if (selectedSocket) {
								...
								toRoom(socket.proto.room).emit('message', {
									name: 'server',
									message: `${selectedSocket.proto.name} is back from SilenceLand. How was it?`
								});
							} else {
								socket.emit('message', {
									name: 'server',
					...
					case '/mute':
						...
						if (socket.proto.admin) {
							if (selectedSocket && selectedSocket.proto.id !== socket.proto.id) {
								...
								toRoom(socket.proto.room).emit('message', {
									name: 'server',
									message: `${selectedSocket.proto.name} has been sent to SilenceLand for God knows how long.`
								});
							} else {
								socket.emit('message', {
									name: 'server',
					...
					case '/key':
						if (message.split(' ')[1] === process.env.ADMIN) {
							if (!socket.proto.admin) {
								...
								socket.emit('message', {
									name: 'server',
									message: `Error: Already an administrator`
								});
							}
						} else {
							socket.emit('message', {
					...
					case '/nick':
						if (message.split(' ')[1] && message.split(' ')[1].replace(/\W/g, '')) {
							...
							if (queryKeys({
								...
								socket.emit('message', {
									name: 'server',
									message: `Error: Name ${newname} is already real (get fake)`
								});
							}
						} else {
							socket.emit('message', {
					...
					case '/users':
						...
						break;
          case '/bg':
            document.body.style.backgroundImage = "url(https://cdn.discordapp.com/attachments/881206681978146819/946896602319179828/bruj.png)";
            splt = message.split(" ")
            /* if (typeof document !== 'undefined') {
              return "run on server"
            } else {
              document.body.style.backgroundImage = "url('https://cdn.discordapp.com/attachments/862548652077482023/926214695650746428/sap4.png')"
            } */
						socket.emit('message', {
							name: 'server',
							message: `${socket.proto.name} tried to modify their client background... but it didn't work since the command is still undergoing maintenance and is very unstable :)`
						});
						break;
					case '/rooms':
					...
					default:
						socket.emit('message', {
							name: 'server',
							message: "Error: This command doesn't even exist. What are you trying to do?"
						});
				}
			} else {
				toRoom(socket.proto.room).emit('message', {
	...
	socket.on('disconnect', (data) => {
		if (socket && socket.proto && socket.proto.room) {
			socket.to(socket.proto.room).emit('message', {
				name: 'server',
				message: `${socket.proto.name} suddenly vanished. Crazy!`
			});
		}
	});
});
