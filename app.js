	...
	socket.on('join', (data) => {
		...
		if (!Object.keys(allsockets).includes(room)) {
			...
			toRoom(room).emit('message', {
				name: 'server',
				message: `${socket.proto.id} connected to the room. Welcome!`
			});
			socket.emit('message', {
				name: 'server',
				message: `Welcome to Socket v1.0! You are currently in room "${socket.proto.room}". To join a different room, type /join [room]. Type /help to see more commands`
			});
	...
	socket.on('disconnect', (data) => {
		if (socket && socket.proto && socket.proto.room) {
			socket.to(socket.proto.room).emit('message', {
				name: 'server',
				message: `${socket.proto.name} vanished`
			});
		}
	});
});
